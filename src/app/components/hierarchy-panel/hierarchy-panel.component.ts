...
export class HierarchyPanelComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  constructor(private zone: NgZone, private cdr: ChangeDetectorRef) {
    try { this.layoutLivre = localStorage.getItem('planner_hierarchy_freelayout') === '1'; } catch { }
  }
  ...
  ngOnChanges(): void { if (this.layoutLivre) this.garantirPosicoesFallback(); this.agendarRecalcLinhas(); }
  ...
  ngOnDestroy(): void {
    ...
    document.removeEventListener('mousemove', this.onCardMouseMove, true);
    document.removeEventListener('mouseup', this.onCardMouseUp, true);
  }
  ...
  onEstruturaDragStart(node: any, ev: DragEvent): void {
    // Se o arraste nasceu de um membro (pessoa), deixa o fluxo de mover-membro cuidar.
    const alvo = ev.target as HTMLElement | null;
    if (alvo && alvo.closest('.node-m')) return;
  ...
  onEstruturaDragEnd(): void {
    this.dragNodeId = null;
    this.dragOverNodeId = '';
    this.dragOverRoot = false;
  }

  /** Alvo válido para receber a estrutura arrastada: não é ela mesma nem uma descendente (evita ciclo). */
  podeReceberEstrutura(targetId: string): boolean {
    if (!this.dragNodeId || targetId === this.dragNodeId) return false;
    return !this.subtree(this.dragNodeId).has(targetId);
  }

  /** Payload de update completo a partir do node atual (preserva tudo). `over` sobrescreve campos. */
  private montarPayloadNode(node: any, over: any = {}): any {
    const n = this.tipoNormalizado(node.tipo);
    return Object.assign({
      id: node.id,
      tipo: n,
      tipoRotulo: node.tipoRotulo || null,
      nome: node.nome,
      descricao: node.descricao || '',
      parentId: this.paisDe(node)[0] || null,
      parentIds: [...this.paisDe(node)],
      ordem: node.ordem,
      membros: (node.membros || []).map((o: any) => ({
        personId: o.personId ?? null, nomePessoa: o.nomePessoa, papel: o.papel || '',
        cross: !!o.cross, vinculo: o.vinculo ?? null,
        percentual: this.tipoPermiteMarcacoesDeTime(n) ? (o.percentual ?? null) : null,
        subgrupo: o.subgrupo ?? null,
      })),
      loIds: [...(node.loIds || [])],
      projetoIds: [...(node.projetoIds || [])],
      projetosOcultos: [...(node.projetosOcultos || [])],
      posX: node.posX ?? null,
      posY: node.posY ?? null,
    }, over);
  }

  /** Reposiciona a estrutura sob os pais informados (lista vazia = vira raiz). Preserva todo o resto. */
  private reparentEstrutura(node: any, parentIds: string[]): void {
    const atuais = this.paisDe(node);
    if (atuais.length === parentIds.length && atuais.every((p: string) => parentIds.includes(p))) return; // já está lá
  ...
  onDocumentKeydown(ev: KeyboardEvent): void {
    ...
    if (!this.vinculoDestacado) return;
  ...
  removerVinculoSelecionado(): void {
    ...
    if (!v) return;
    const filho = this.nodes.find((n: any) => n.id === v.para);
    this.vinculoDestacado = null;
    if (!filho) return;
    const novosPais = this.paisDe(filho).filter((p: string) => p !== v.de);
  ...
  onToggleLayoutLivre(): void {
    try { localStorage.setItem('planner_hierarchy_freelayout', this.layoutLivre ? '1' : '0'); } catch { }
    // Ao LIGAR: semeia a posição de cada card a partir de onde ele já está na tela
    // (o DOM ainda mostra o layout automático neste instante), para não reorganizar nada.
    if (this.layoutLivre) this.capturarPosicoesAtuais();
    this.onHierarchyLayoutChange();
  }

  /** Captura a posição atual (lógica, dentro de .tree) de cada card visível para usar como ponto de partida. */
  private capturarPosicoesAtuais(): void {
    const cont = this.treeContainer?.nativeElement;
    if (!cont) return;
    const crect = cont.getBoundingClientRect();
    const z = this.zoom || 1;
    const cache: Record<string, { x: number; y: number }> = { ...this.posCache };
    for (const n of this.nosVisiveisFlat()) {
      const el = cont.querySelector<HTMLElement>(`.node-card[data-node-id="${n.id}"]`);
      if (!el) continue;
      const r = el.getBoundingClientRect();
      cache[n.id] = { x: (r.left - crect.left) / z, y: (r.top - crect.top) / z };
    }
    this.posCache = cache;
  }

  /** Nós visíveis (respeita cadeias ocultas e a raiz de exportação), em lista plana. */
  nosVisiveisFlat(): any[] {
    this.ensureHiddenLoaded();
    let vis = (this.nodes || []).filter((n: any) => !this.estaEmCadeiaOculta(n));
    if (this.exportRootId) {
      const root = this.nodes.find((n: any) => n.id === this.exportRootId);
      if (root) { const sub = this.subtree(root.id); vis = vis.filter((n: any) => sub.has(n.id)); }
    }
    return vis;
  }

  /**
   * Preenche uma posição de fallback APENAS para nós sem posição salva e ainda não semeados
   * (ex.: recarregou já no modo livre, ou estrutura recém-criada). Nunca move quem já tem lugar.
   */
  private garantirPosicoesFallback(): void {
    const vis = this.nosVisiveisFlat();
    const byId = new Map(vis.map((n: any) => [n.id, n]));
    const nivel: Record<string, number> = {};
    const calc = (id: string, seen: Set<string>): number => {
      if (nivel[id] != null) return nivel[id];
      if (seen.has(id)) return 0;
      seen.add(id);
      let l = 0;
      for (const p of this.paisDe(byId.get(id)).filter((p: string) => byId.has(p))) l = Math.max(l, calc(p, seen) + 1);
      nivel[id] = l; return l;
    };
    const colDoNivel: Record<number, number> = {};
    const stepX = 280, stepY = 210, padX = 40, padY = 40;
    const ordenados = [...vis].sort((a: any, b: any) => (calc(a.id, new Set()) - calc(b.id, new Set())) || this.byOrdem(a, b));
    for (const n of ordenados) {
      const px = Number(n?.posX), py = Number(n?.posY);
      if (Number.isFinite(px) && Number.isFinite(py)) continue; // já tem posição salva
      if (this.posCache[n.id]) continue;                        // já semeado
      const lv = nivel[n.id] || 0;
      const col = colDoNivel[lv] ?? 0; colDoNivel[lv] = col + 1;
      this.posCache[n.id] = { x: padX + col * stepX, y: padY + lv * stepY };
    }
  }
  ...
  posDe(node: any): { x: number; y: number } {
    ...
    return this.posCache[node.id] || { x: 40, y: 40 };
  }

  /** Dimensão mínima do canvas livre para caber todos os cards (com folga para rolagem). */
  freeCanvasSize(): { w: number; h: number } {
    let maxX = 0, maxY = 0;
    for (const n of this.nosVisiveisFlat()) { const p = this.posDe(n); if (p.x > maxX) maxX = p.x; if (p.y > maxY) maxY = p.y; }
    return { w: maxX + 360, h: maxY + 320 };
  }

  // Arrastar o card (ponteiro) para movê-lo livremente — só no modo layout livre.
  onCardMouseDown(node: any, ev: MouseEvent): void {
    if (!this.layoutLivre || ev.button !== 0) return;
    const t = ev.target as HTMLElement | null;
    if (t && t.closest('button, input, select, textarea, a, .node-m')) return; // handles internos
    ev.preventDefault();
    ev.stopPropagation(); // não deixa o pan da árvore começar
    const start = this.posDe(node);
    this.freeDrag = { id: node.id, x: start.x, y: start.y, offX: ev.clientX, offY: ev.clientY, baseX: start.x, baseY: start.y };
    this.zone.runOutsideAngular(() => {
      document.addEventListener('mousemove', this.onCardMouseMove, true);
      document.addEventListener('mouseup', this.onCardMouseUp, true);
    });
  }
  private onCardMouseMove = (ev: MouseEvent): void => {
    if (!this.freeDrag) return;
    const z = this.zoom || 1;
    this.freeDrag.x = Math.max(0, this.freeDrag.baseX + (ev.clientX - this.freeDrag.offX) / z);
    this.freeDrag.y = Math.max(0, this.freeDrag.baseY + (ev.clientY - this.freeDrag.offY) / z);
    this.zone.run(() => { this.agendarRecalcLinhas(); this.cdr.markForCheck(); });
  };
  private onCardMouseUp = (): void => {
    const fd = this.freeDrag;
    document.removeEventListener('mousemove', this.onCardMouseMove, true);
    document.removeEventListener('mouseup', this.onCardMouseUp, true);
    this.freeDrag = null;
    if (!fd) return;
    const node = this.nodes.find((n: any) => n.id === fd.id);
    // Só persiste se realmente mudou de lugar.
    if (node && (Math.round(fd.x) !== Math.round(fd.baseX) || Math.round(fd.y) !== Math.round(fd.baseY))) {
      this.zone.run(() => this.update.emit(this.montarPayloadNode(node, { posX: Math.round(fd.x), posY: Math.round(fd.y) })));
    } else {
      this.zone.run(() => this.cdr.markForCheck());
    }
  };

  // Soltar em qualquer área livre da árvore torna a estrutura uma raiz (fica "solta", sem estrutura superior).
  onCanvasDragOver(ev: DragEvent): void {
    if (!this.dragNodeId) return;
    // Sobre um card, quem manda é o drop do card (re-parent). Aqui tratamos só o espaço vazio.
    if ((ev.target as HTMLElement | null)?.closest('.node-card')) { this.dragOverRoot = false; return; }
    ev.preventDefault();
    if (ev.dataTransfer) ev.dataTransfer.dropEffect = 'move';
    this.dragOverRoot = true;
    this.dragOverNodeId = '';
  }
  onCanvasDragLeave(ev: DragEvent): void {
    // Só limpa quando o ponteiro realmente saiu do container (não ao passar por cima de um filho).
  ...
  onCanvasDrop(ev: DragEvent): void {
    if (!this.dragNodeId) return;
    if ((ev.target as HTMLElement | null)?.closest('.node-card')) return; // card já cuidou
    ev.preventDefault();
    const node = this.nodes.find((n: any) => n.id === this.dragNodeId);
    this.dragOverRoot = false;
    this.dragNodeId = null;
    if (node) this.reparentEstrutura(node, []);
  }
