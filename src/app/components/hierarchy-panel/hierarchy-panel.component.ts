...
export class HierarchyPanelComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  fteTotalNode(node: any): number {
    ...
    let total = 0;
    for (const n of this.nodes) {
      if (!ids.has(n.id) || !this.mostrarMarcacoesDeTime(n)) continue;
      total += (n.membros || []).reduce((s: number, m: any) => s + this.fteMembro(m), 0);
    }
    return total;
  }
  ...
  mostrarFteTotal(node: any): boolean {
    ...
    const ids = this.subtree(node.id);
    return this.nodes.some((n: any) => ids.has(n.id) && this.mostrarMarcacoesDeTime(n) && (n.membros || []).length > 0);
  }
