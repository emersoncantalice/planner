  ...
  ftePorPerfilNode(node: any): Array<{ label: string; fte: number }> {
    ...
    for (const n of this.nodes) {
      ...
      for (const m of (n.membros || [])) {
        ...
        // Dev Team só soma quem conta FTE no cadastro; papéis nomeados contam sempre.
        acc[g] += (g === 'Dev Team') ? (this.membroContaFte(m) ? base : 0) : base;
      }
    }
